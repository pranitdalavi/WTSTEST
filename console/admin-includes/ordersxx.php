...
<form id="status_form" action="" method="get"> <input type="hidden" name="page" value="orders"> <select class="form-control" id="status" name="status"> <option value="New" <?php if($_GET['status'] == 'New'){ print 'selected'; } ?>>New Orders</option> <option value="Completed" <?php if($_GET['status'] == 'Completed'){ print 'selected'; } ?>>Completed Orders</option> <option value="Dispatched" <?php if($_GET['status'] == 'Dispatched'){ print 'selected'; } ?>>Dispatched Orders</option> <option value="Pending" <?php if($_GET['status'] == 'Pending'){ print 'selected'; } ?>>Pending Orders (not paid)</option> </select> </form>
	
	</div>
	
	<form action="" method="get">

<div class="col-md-3"> <input type="hidden" name="page" value="orders"> <input type="hidden" name="status" value="<?= $status ?>"> <input placeholder="order number, surname, email, postcode, phone..." type="text" name="search" class="form-control" value="<?php if(isset($_GET['search'])){ print $_GET['search']; } ?>">  	</div>
	...
	<form action="" method="get">
		...
		<div class="col-md-1">
			...
			<input required name="from" type="text" class="form-control datepicker" placeholder="From">
		
		</div>
		
		<div class="col-md-1">
		
			<input required name="to" type="text" class="form-control datepicker" placeholder="To">
...
<div class="table-responsive">
	...
	if($row->promo_code_id){
		...
		if( $user->isAdmin() ){
			...
			print '<tr><td>'.date('d/m/Y H:i', strtotime($row->order_date)).'</td><td>'.$status.'</td><td>'.$row->order_number.'</td><td>£'.number_format($cost, 2).'</td><td><a class="btn btn-primary" href="account.php?page=order&id='.$row->order_id.'">View <i class="fa fa-arrow-right"></i></a></td></tr>';
