...
class Tools
{
	...
	foreach($_SESSION as $key => $session){
		...
		if( $key != SESSION.'SID' && $key != SESSION.'shipping' && $key != SESSION.'unique'  ){
		...
		if($_FILES[$key]['size'] > 0){
			...
			$size = getimagesize($_FILES[$key]['tmp_name']);
				...
				if ( $ext != "jpg" && $ext != "png" && $ext != "gif" && $ext != "jpeg" ){
					...
					return redirect( $_SERVER['REQUEST_URI'], 'JPG, JPEG, PNG or GIF extensions only', 'e' );
    ...
    public function resize($uploadedfile, $ext, $image_folder, $image_id)
    {
					...
					$filename = '../'.$image_folder.'-thumbnails/'.$image_id.'.'.$ext;
					
							if($ext == 'gif'){
								imagegif($tmp,$filename);
							} elseif ($ext == 'png'){
								imagepng($tmp,$filename,9, PNG_ALL_FILTERS);
							}else{
					...
					if($width > 1920){
						...
						imagecopyresampled($tmp,$src,0,0,0,0,$newwidth,$newheight,$width,$height);
						$filename = '../'.$image_folder.'/'.$image_id.'.'.$ext;
	...
	if(in_array($key, $uploadedArray)){ continue; }
		...
		if($_FILES[$key]['size'] > 0){
			...
			$alt_key = str_replace('file', 'alt', $key);
			$alt = $_POST[$alt_key];
			$type_key = str_replace('file', 'type', $key);
			$type = $_POST[$type_key];
			
			$ext = self::getFileExtension($_FILES[$key]['name']);
		
			$image_id = $obj->addImage( $id, $alt, $ext, $type );
