...
class AttributeImage extends ObjectModel
{
    ...
    public function getAll($blog_id = null)
    {
        ...
        return $this->execute('SELECT * FROM attribute_images WHERE blog_id = ? AND deleted_at IS NULL ORDER BY id ASC ', [ $blog_id ] );
    }

    public function getAllSmallImages($blog_id = null)
    {
        $blog_id = $blog_id ? $blog_id : $_GET['id'];
        return $this->execute('SELECT * FROM attribute_images WHERE blog_id = ? AND deleted_at IS NULL ORDER BY rand() LIMIT 10', [ $blog_id ] );
    }
